...
func NewRecoverer(client rediswrapper.Redis, locker dlock.Lock) Recoverer {
	return &recoverer{
		client: client,
		locker: locker,
	}
...
func (r *recoverer) Recover() error {
	...
	results, err := r.client.LRange(constants.InProcessQueue, 0, 0)
	...
	_, err = r.client.RPush(constants.WorkerQueueName(job.Type), results[0])
	if err != nil {
		return errors.Wrap(err, "Unable to push job %s to worker queue")
	}
	_, err = r.client.LRem(constants.InProcessQueue, 1, results[0])
	if err != nil {
